...
class LearningAgent(Agent):
    ...
    def createQ(self, state):
        ...
        if self.learning and state not in self.Q:
            current_state_actions = dict()
            for action in self.valid_actions:
                current_state_actions[action] = 0.0
            self.Q[state] = current_state_actions
...
def run():
    ...
    env = Environment()
    # env = Environment(verbose=False, num_dummies=25, grid_size=(5, 5))
